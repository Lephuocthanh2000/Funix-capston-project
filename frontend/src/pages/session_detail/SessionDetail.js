...
const SessionDetail = () => {
  ...
  const countdownRef = useRef(null) // Hoặc khởi tạo giá trị mặc định của useRef
  ...
  const getProposedPrice = async () => {
    let contract = await loadContractWithProvider(address)
    let participantProposedPrice = await contract.sessionProposes(
      wallet.accounts[0]
    )
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (countdownRef.current && countdownRef.current.api) {
                ...
                >
                  <div className="col s2">
                    <img
                      ...
                      await
                      src={`https://cf-ipfs.com/ipfs/${productImages[0]}`}
                      alt=""
