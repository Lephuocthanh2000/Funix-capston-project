...
const Register = () => {
  ...
  const isConnected = Boolean(wallet.accounts && wallet.accounts[0])
  ...
  useEffect(() => {
    if (isConnected) {
      checkIsRegistered().then((result) => {
        if (result) {
          M.toast({
            html: 'This address has registered. Redirect to account ...',
            classes: 'rounded',
          })
          setTimeout(
            () =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let tx = await contract.register(name, email, {
